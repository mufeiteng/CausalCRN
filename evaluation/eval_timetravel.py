...
from CausalCRN.evaluation.entail_client.nli_metrics.entail_score import EntailScore
# from ctrltextgen.evaluation.distinct import distinct_n_corpus_level
...
def load_json_to_instances(samples):
    ...
    for d in samples:
        ...
        predicted_ending = d['generated_endings']
        instance = CFRInstance(
            original_context=' '.join((premise, initial)),
            cf_context=' '.join((premise, counterfactual)),
...
def eval_nli(instances, model_path,bs):
    nli_scorer = EntailScore(model_path)
    score, score_by_instance = nli_scorer.score_from_jsonl(instances,bs=bs)
    return {
        'entail_score': score,
...
def do_eval_from_instances(instances, metrics, bert_model='bert-base-uncased', entail_model="", bs=2):
    results = {}
    if 'bleu' in metrics:
    ...
    if 'entailscore' in metrics:
        results.update(eval_nli(instances, entail_model,bs=bs))
        if 'bleu' in metrics:
...
#     parser.add_argument('--metrics', '-m', nargs='+', default=['bertscore', 'bleu', 'rouge_', 'entailscore'])
#     args = parser.parse_args()
#
#     if not args.only_assemble:
#         results = do_eval_from_instances(args.pred_txt, args.input_json, args.metrics)
#         print(results)
#
